...
def main(config: DictConfig) -> None:
    ...
    # Load the dataset
    # Create random data
    embeddings = torch.tensor(np.load("data/train_embeddings.npy"), dtype=torch.float32)
    label2 = torch.tensor(np.load("data/train_age_labels.npy"), dtype=torch.float32)
    label1 = torch.tensor(np.load("data/train_gender_labels.npy"), dtype=torch.float32)
    print(
        embeddings.shape,
        label1.shape,
        label2.shape,
